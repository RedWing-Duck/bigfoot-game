...
const alibi = (a, words = a) => ({ if: { flag: "checkpoint", said: words, min: { [a]: 1 }, max: { [a]: 1 } }, ...CLEAR, add: { [a]: 1, cleared: 1 }, say: T.guard[a] });
...
const G = {
  ...
  rooms: {
    ...
    salon: { name: "Music Salon", desc: desc("salon"), exits: exits("salon", { west: "foyer" }),                // R4
      on: merge(strikes("piano|lid|keys|keyboard|ivory|ivories", "PIANO"), { play: [
        ...
        { if: { not: { flag: "p1" } }, say: T.p1.wrong, feedback: true },
    ...
    atrium: { name: "Atrium", desc: desc("atrium"), exits: exits("atrium", { south: "gallery", north: "aviary" }),   // R8
      on: merge(strikes(HIPPO, "HIPPO"), {
        ...
        ...Object.fromEntries(["Take", "touch", "open", "push"].map(v => [v, [{ if: { ...ESC, said: HIPPO }, say: T.p2.wrong, feedback: true }]])) }) },
  ...
  events: [
    ...
    { if: { flag: "checkpoint", since: { stop: 1 } }, end: [...Object.keys(T.nando.caught).map(k => ({ if: { key: "CAUGHT_BY=" + k },
      text: T.end.L2.replace("CAUGHT\n\n", "CAUGHT\n\n" + T.nando.caught[k] + "\n\n") + T.retry })), T.end.L2 + T.retry] },
